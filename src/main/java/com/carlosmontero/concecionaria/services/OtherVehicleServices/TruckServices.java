...
@Service
public class TruckServices extends VehicleServiceImpl<Truck> {

    protected List<Truck> truckList = new ArrayList<>();
    ...
    public List<Truck> getTrucksByCapacity(int capacity) {
        truckList.clear();

        for (Truck t : vehicles) {
            if (t.getLoadCapacity() >= capacity) {
                truckList.add(t);
            }
        }
        return truckList;
    ...
    public List<Truck> getTrucksByAxis(int numAxis) {
        truckList.clear();

        for (Truck t : vehicles) {
            if (t.getNumberOfAxles() == numAxis) {
                truckList.add(t);
            }
        }
        return truckList;
    ...
    public List<Truck> getTrucksByPermit(String permit) {
        truckList.clear();
        boolean permitBool = Boolean.parseBoolean(permit.toLowerCase());

        for (Truck t : vehicles) {
            if (t.isSpecialPermitRequired() == permitBool) {
                truckList.add(t);
            }
        }
        return truckList;
    ...
    public List<Truck> getTrucksByTowingSystem(String towingSystem) {
        truckList.clear();
        boolean towingSystemBool = Boolean.parseBoolean(towingSystem.toLowerCase());

        for(Truck t: vehicles){
            if(t.isHasTowingSystem() == towingSystemBool){
                truckList.add(t);
            }
        }
        return truckList;
